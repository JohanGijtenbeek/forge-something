...
public class ArticleCreatedConsumer : IConsumer<ArticleCreatedEvent>
{
    ...
    public ArticleCreatedConsumer(DbConnectionFactory factory, ISearchService search, IHubContext<EventHub> hub)
    {
        _factory = factory;
        _search = search;
        _hub = hub;
    ...
    public async Task Consume(ConsumeContext<ArticleCreatedEvent> context)
    {
            ...
            try
            {
                ...
                await conn.ExecuteAsync(@"
                    ...
                    new
                    {
                        ...
                        EventType = "ArticleCreated",
                        Summary = $"Article created: {e.Code} - {e.Name}",
                        ChangedBy = "system",
        ...
        await _search.IndexArticleAsync(new ArticleSearchDocument(
            e.ArticleId.ToString(), e.Code, e.Name, null, true));
